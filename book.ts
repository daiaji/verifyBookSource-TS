...
export class Book {
    static headers = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36 Edg/114.0.1823.58' };
    file: string;
    errorOutput: boolean;
    type: string | null;
    ...
    async jsonToBooks(): Promise<BookSource[]> {
        try {
            ...
            console.error(`解析文件时发生错误，文件: ${this.file}，错误信息: ${error}`);
            return [];
        }
    }
    ...
    async check(abook: BookSource, timeout = 5000): Promise<BookResult> {
        try {
            const [url, json] = abook.searchUrl.split(',', 2);
            const searchUrl = JSON.parse(json.replace("{{key}}", "我的"));
            ...
            const response = await fetchWithTimeout(abook.bookSourceUrl + url, {
                ...
                headers,
                body: searchUrl.body ? encode(searchUrl.body, searchUrl.charset) : undefined
            });
            if (response.ok) {
            ...
            if (this.errorOutput) {
                console.error(`检查书源时发生错误，错误信息: ${(error as any).message}，URL: ${abook.bookSourceUrl}`);
            }
            return { book: abook, status: false };
        }
    }

    async checkBooks(workers: number): Promise<{ good: BookResult[], error: BookResult[] }> {
        ...
        for (let i = 0; i < books.length; i += workers) {
            const promises = books.slice(i, i + workers).map((book: BookSource) => this.check(book));
            const res = await Promise.allSettled(promises);
            results.push(...res.filter((result: PromiseSettledResult<BookResult>) => result.status === 'fulfilled') as PromiseFulfilledResult<BookResult>[]);
        }
        const good = results.filter((result: PromiseFulfilledResult<BookResult>) => result.value.status).map((result: PromiseFulfilledResult<BookResult>) => result.value);
        const error = results.filter((result: PromiseFulfilledResult<BookResult>) => !result.value.status).map((result: PromiseFulfilledResult<BookResult>) => result.value);
        return { good, error };
    }
